...
//////////////3/////////////
const countFacultyMembers=async()=>{
...
/////////////4////////////////
const phd= async()=>{
  const selectedMembers= await Faculty.find({qualification: "Ph.D"})
...
/////////////////5////////////
const addOne=async()=>{
  Faculty.insertMany(member)
}
// addOne()

/////////////6/////////////////
const updateSivani=async()=>{
  await Faculty.findOneAndUpdate({name:"Sivani"}, {qualification: "Ph.D", type: "Full Time"})
}
// updateSivani()

///////////////7////////////////
const deleteMani=async()=>{
  await Faculty.findOneAndDelete({name: "Mani"})
}
// deleteMani()

///////////////8///////////////////
const deleteOldOnes=async()=>{
  await Faculty.deleteMany({age: {$gt: 39}})
}
// deleteOldOnes()
...
app.get("/", async(req, res)=>{
  const allMembers= await Faculty.find()
  res.json(allMembers)
})